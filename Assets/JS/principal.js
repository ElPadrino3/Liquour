class TouchTexture {
    ...
    drawPoint(point) {
        ...
        this.ctx.shadowBlur = this.radius * intensity;
...
class App {
    ...
    init() {
        ...
        this.uniforms = {
            ...
            uTouchTexture: { value: this.touch.texture },
            // Colores: Dorado Licor, Ámbar Profundo y Azul Noche
            uColor1: { value: new THREE.Vector3(0.89, 0.75, 0.35) }, 
            uColor2: { value: new THREE.Vector3(0.45, 0.25, 0.05) },
            uColor3: { value: new THREE.Vector3(0.02, 0.02, 0.08) }
        };
        ...
        const material = new THREE.ShaderMaterial({
            ...
            fragmentShader: `
                ...
                void main() {
                    ...
                    // Distorsión líquida basada en touch
                    uv.x += touch.r * 0.15;
                    uv.y += touch.g * 0.15;

                    float t = uTime * 0.3;
                    float noise = sin(uv.x * 4.0 + t) * cos(uv.y * 3.0 - t);
                    noise += sin(uv.y * 7.0 + t * 1.5) * 0.2;

                    vec3 color = mix(uColor3, uColor2, noise * 0.5 + 0.5);
                    color = mix(color, uColor1, touch.b * 0.8);
    ...
    addEvents() {
        window.addEventListener("mousemove", (e) => {
            this.touch.addTouch({ x: e.clientX / window.innerWidth, y: 1 - e.clientY / window.innerHeight });
            if(this.cursorEl) {
